...
class Parser {
  ...
  static Map<Course,Map<Section,List<Question>>> parse(String raw) {
    ...
    Map<Course,Map<Section,List<Question>>> map = Map();
    
    List<String> lines = [for (String line in raw.split('\n')) if (line.isNotEmpty) line.trim()];

    // Skip to first course.
    lines = lines.skipWhile((line) => !line.startsWith('# ')).toList();
    
    while(lines.isNotEmpty) {
      Course course = Course(title: lines.first.substring(2));
      map[course] = Map();
      // Skip course title.
      lines = lines.skip(1).toList();
      // Take all lines before the next course and skip to first section
      List<String> courseLines = lines.takeWhile(isNotCourseTitle).skipWhile(isNotSectionTitle).toList();

      while(courseLines.isNotEmpty) {
        Section section = Section(title: courseLines.first.substring(3));
        map[course][section] = List<Question>();
        // Skip section title.
        courseLines = courseLines.skip(1).toList();
        // Take all lines before the next section and remove invalid lines.
        List<String> sectionLines = courseLines.takeWhile(isNotSectionTitle).where(isQuestionText).toList();
        for (String line in sectionLines) {
          map[course][section].add(Question(text: line.substring(2)));
        }
